...
public class TreeFactory extends AbstractCandidateFactory<Node>
{
    ...
    private Node makeNode(Random rng, int maxDepth)
    {
        if (functionProbability.nextEvent(rng) && maxDepth > 1)
        {
            ...
            {
            	case 0: return new Sine(makeNode(rng, depth));
            	case 1: return new Cosine(makeNode(rng, depth));
            	case 2: return new Addition(makeNode(rng, depth), makeNode(rng, depth));
                case 3: return new Subtraction(makeNode(rng, depth), makeNode(rng, depth));
                case 4: return new Multiplication(makeNode(rng, depth), makeNode(rng, depth));
                //case 5: return new Division(makeNode(rng, depth), makeNode(rng, depth));
                case 6: return new IsGreater(makeNode(rng, depth), makeNode(rng, depth));
                case 7: return new IsEqual(makeNode(rng, depth), makeNode(rng, depth));
                case 8: return new IsNotEqual(makeNode(rng, depth), makeNode(rng, depth));
                case 9: return new IsLess(makeNode(rng, depth), makeNode(rng, depth));
                case 10: return new Negate(makeNode(rng, depth));
                case 11: return new Tangent(makeNode(rng, depth));
                case 12: return new Absolute(makeNode(rng, depth));
                default: return new IfThenElse(makeNode(rng, depth), makeNode(rng, depth), makeNode(rng, depth));
            }
        }
        ...
        {
            return new Constant(rng.nextInt(255));
        }
    }
}
